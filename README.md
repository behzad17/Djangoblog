...
Default provider is **`mock`** (no network). To use OpenAI (Responses API), set:
...
Internal AI Integration API (staff/superuser): `POST /api/internal/ai/editorial/draft/` (returns in-memory draft + telemetry; no Blog save)
